...
// 运行时的核心部分，负责初始化Go程序的运行时环境（）、创建系统Goroutine、执行main.main（用户协程），并在 main.main 结束后进行清理和退出。
func main() {
	g := getg() // g = 系统goroutine，不是g0
	...
	// 在初始化期间，将主 goroutine（也就是运行 main.main 的那个 goroutine）锁定在主操作系统线程上。
	// 大多数程序对此不用关心，但少数程序的某些调用确实需要由主线程调用。
	// 那些有此需求的程序可以在初始化时调用 runtime.LockOSThread，以确保 main.main 在主线程上运行并保持该绑定。
	lockOSThread()

	if g.m != &m0 { //确保当前g所在的m是m0
		throw("runtime.main not on m0")
	}

	// Record when the world started.
	// Must be before doInit for tracing init.
	runtimeInitTime = nanotime() //系统启动的时间
	...
	// 使用defer方式来解除g和m的绑定，这是为了在初始化期间，若有异常或调用runtime.Goexit()的方式退出时，能确保gm解绑
	needUnlock := true
	defer func() {
		if needUnlock {
			unlockOSThread() //解除gm绑定
		}
	}()
	...
	if iscgo {
		...
		// 启动一个模板线程（所谓模板线程就是预先创建好的线程），以便于当
		// 从C创建的线程进入/调用Go代码时，需要创建新线程（Go线程）的情况
		startTemplateThread()
	...
	//调用main.main函数（用户的入口函数）
	fn := main_main // make an indirect call, as the linker doesn't know the address of the main package when laying down the runtime
	fn()            //执行用户定义的 main.main()

	//main.main函数返回，也即程序运行结束，下面是一些收尾工作
	...
	if atomic.Load(&panicking) != 0 { //如果还有panic未进行recover处理
...
// 启动强制gc协程，该协程负责监听是否需要启动gc
func init() {
	go forcegchelper() //GC监听启动
}

func forcegchelper() {
	forcegc.g = getg() //获取当前g（该g并定不是g0，因为看L302）
	...
	for {
		...
		gcStart(gcTrigger{kind: gcTriggerTime, now: nanotime()}) //定时触发GC，标记+清除都是并发模式
	}
}
...
// 把当前的用户协程g设为waiting态并且在系统栈g0调用unlockf函数，这个函数的作用是解锁+判断是否可以把该用户协程g立即转回runnable
// 当然如果unlockf函数里涉及到有阻塞当前M（系统线程）的系统调用，那么整个M会被阻塞挂起
// 如果 unlockf 返回false，则g立即转回runnable，否则g被挂起并设置为waiting
func gopark(unlockf func(*g, unsafe.Pointer) bool, lock unsafe.Pointer, reason waitReason, traceEv byte, traceskip int) {
	...
	if status != _Grunning && status != _Gscanrunning { //如果当前的g并不是running或_Gscanrunning态，那么调用gopark就是出错了
	...
	mcall(park_m) //切换到g0栈（系统线程栈）上执行park_m，里面会调用 mp.waitunlockf(gp, mp.waitlock)
}

// Puts the current goroutine into a waiting state and unlocks the lock.
// The goroutine can be made runnable again by calling goready(gp).
// 把当前goroutine设为waiting态并且解锁lock，该goroutine可以通过 goready 函数恢复到 runnable态
...
var (
	...
	allglen uintptr //存放allgs的len
	allgptr **g     //指向allgs的地址
)

// 把gp放入到allg全局链表里，以便GC回收
func allgadd(gp *g) {
	if readgstatus(gp) == _Gidle { //如果G处于 _Gidle态，则不回收
...
// 返回&allgs[0]（地址）和len(allgs)，以便 atomicAllGIndex的使用
...
// forEachGRace函数，在allgs里的每个G上执行fn函数
// forEachGRace避免了锁
...
func schedinit() {
	...
	mcommoninit(_g_.m, -1) //初始化m0，因为从前面的代码我们知道g0->m = &m0，也即g0当前的M就是m0
...
// 1.设置新m的标识（id）；
// 2.创建用于处理信号的gsignal
// 3.把刚新建的M（mp）添加到全局链表队列 allm
func mcommoninit(mp *m, id int64) {
	_g_ := getg()

	// g0 stack won't make sense for user (and is not necessary unwindable).
	if _g_ != _g_.m.g0 { //如果当前的g不是g0
	...
	//创建用于处理信号的gsignal（也是G来的），只是简单的从堆上分配一个G对象,然后把栈设置好就返回了
	mpreinit(mp)
	if mp.gsignal != nil {
	...
	// 通过 「mp.alllink = allm」 和 「atomicstorep(unsafe.Pointer(&allm), unsafe.Pointer(mp))」这两步
	// 实现了一个链表添加新结点的操作（从链头添加新结点），先加入的结点排在链表的尾部
...
func ready(gp *g, traceskip int, next bool) {
	...
	// 标记gp为runnable
	_g0_ := getg()   //g0
	mp := acquirem() // 关闭抢占 disable preemption because it can be holding p in a local var
	if status&^_Gscan != _Gwaiting {
		dumpgstatus(gp)
		throw("bad g->status in ready")
	}

	// gp当前状态为 Gwaiting 或 Gscanwaiting, 把它标记为 Grunnable 然后放入runq运行队列
	casgstatus(gp, _Gwaiting, _Grunnable)
	runqput(_g0_.m.p.ptr(), gp, next) //把gp放入当前m所绑p的本地队列里（该队列里的g都是runnable态）
	wakep()                           //尝试调度空闲的p来执行gp
	releasem(mp)                      //
}
...
// 把gp设为_Gscan态
// 如果gp的状态不是期望态时，并且 cas 失败，那么本函数返回false
// castogscanstatus函数的作用和 获取锁 类似，而casfromgstatus函数的作用和 释放锁 类似
func castogscanstatus(gp *g, oldval, newval uint32) bool {
	...
	case _Grunnable,
		...
		_Gwaiting,
		_Gsyscall:
		if newval == oldval|_Gscan { //如果newval态是有_Gscan标识
			r := atomic.Cas(&gp.atomicstatus, oldval, newval) //切换gp当前状态标识
			if r {
				acquireLockRank(lockRankGscan) //获取 lockRankGscan 等级锁
			}
			return r
...
func mstart0() {
	...
	// Go code.
	// 初始化协程栈的stackguard0字段，这样才能调用常规的Go代码
	_g_.stackguard0 = _g_.stack.lo + _StackGuard
	// This is the g0, so we can also call go:systemstack
	// functions, which check stackguard1.
	// 当前处于g0，所以可以调用go:systemstack函数，检查stackguard1
...
// id参数是可选的预分配id（为M的标识），传入-1时底层会调用 mReserveID 获取一个id
// 本函数运行在g0上
//go:yeswritebarrierrec
func allocm(_p_ *p, fn func(), id int64) *m {
	...
	if _g_.m.p == 0 { //如果当前正在运行的g没有挂载的p，则说明是g0，那么就借用p来绑定g0的m（系统线程）
		acquirep(_p_) // 临时借用p来执行分配逻辑 temporarily borrow p for mallocs in this function
	}
	...
	if sched.freem != nil {
		...
		for freem := sched.freem; freem != nil; {
			...
			if wait == freeMWait { //该m还在使用，所以不释放
				next := freem.freelink
			...
			if wait == freeMStack {
				...
				// startm.
				systemstack(func() {
					stackfree(freem.g0.stack) //释放g0对应的栈空间
				})
			}
	...
	mcommoninit(mp, id) //初始化刚新建的M

	// In case of cgo or Solaris or illumos or Darwin, pthread_create will make us a stack.
	// Windows and Plan 9 will layout sched stack on OS stack.
	if iscgo || mStackIsSystemAllocated() {
		mp.g0 = malg(-1) //新建一个带有stacksize字节栈空间的g（new(g)），如果m的栈是有系统分配的，则stacksize传入-1
	} else {
...
// 当一个 cgo 回调发生在一个没有关联 M 的线程上（即，一个不是由 Go 创建的线程），会调用 needm 函数。
// 当一个回调函数在一个非 Go 语言创建的线程（即没有关联 M 的线程）上执行时，
// Go 语言需要将这个线程纳入其调度器的管理中，这就是 needm 函数的作用。
...
func needm() {
	...
	extraMCount--
	unlockextra(mp.schedlink.ptr()) //0
...
// newextram 创建一些M并把它们放到extra list里，本函数在本地的working M里被调用
// 这样就能做一些如调用 schedlock 和 allocate 的事情
func newextram() {
	c := atomic.Xchg(&extraMWaiters, 0) //把extraMWaiters清空
	if c > 0 {                          //原来extraMWaiters有值
...
// oneNewExtraM 分配一个M并把该M放到extra list里
func oneNewExtraM() {
	...
	mp := allocm(nil, nil, -1) //分配一个新M
	gp := malg(4096)           //新建有4KB栈空间的g
	...
	// malg函数返回的G是处于_Gidle态的，在把该G添加到 allg 前改为 _Gdead态，
	...
	// has the same effect.
	atomic.Xadd(&sched.ngsys, +1) //统计系统协程（g0）的数量

	// Add m to the extra list.
	mnext := lockextra(true)
	mp.schedlink.set(mnext)
	extraMCount++
	unlockextra(mp) //把全局空闲M池和extram关联
}
...
var extram uintptr     //0 或 1，作为锁标识
...
// lockextra 锁上extra list并且返回表头，调用方必须通过添加新的表头到extram来解锁list
// 如果nilokay是true，当表头为nil时则会返回nil，当nilokay是false，本函数会一直等待，知道表头不为nil时才返回
//go:nosplit
func lockextra(nilokay bool) *m {
	...
	incr := false
	for {
		old := atomic.Loaduintptr(&extram) //old可能是0，1，或其他
		if old == locked {                 //已上锁，等待锁
			osyield_no_g() //让出当前线程的执行权，允许操作系统调度其他线程运行，减少 CPU 的无效循环
			continue
		}
		if old == 0 && !nilokay { //获取到锁了但是表头为空，则进入等待
			if !incr {
				// Add 1 to the number of threads
				// waiting for an M.
				// This is cleared by newextram.
				// 增加线程等待M的次数（指标统计用），该指标会被newextram函数重置
				atomic.Xadd(&extraMWaiters, 1)
				incr = true
			}
			usleep_no_g(1) //线程休眠指定时间，避免频繁的轮询操作
			continue
		}
		if atomic.Casuintptr(&extram, old, locked) { //获取到锁了
			return (*m)(unsafe.Pointer(old)) //返回表头（可能是nil也可能不是nil）
		}
		osyield_no_g()
		continue
	}
...
var newmHandoff struct {
	...
	// to 1.
	// haveTemplateThread 当为1时，代表 templateThread模板线程已经启动
	// 该字段不会被lock锁保护，使用cas原子设置成1
...
//
// id is optional pre-allocated m ID. Omit by passing -1.
// 创建一个新的m，该m以fn函数为起始点 或 以调度器为起始点 运行（fn是线程的入口函数）
// 该fn必须是静态的并且不是在堆里分配的闭包函数
// m.p可能为nil，所以不可以开启写屏障
// id 是m的id标识，该参数是可选的，默认传入-1
//
//go:nowritebarrierrec
func newm(fn func(), _p_ *p, id int64) {
	...
	if gp := getg(); gp != nil && gp.m != nil && (gp.m.lockedExt != 0 || gp.m.incgo) && GOOS != "plan9" {
		...
		// the thread for us.
		// 当前我们在一个上锁的M 或 由C启动的线程上。当前线程的内核状态可能比较奇怪
...
// startTemplateThread 启动模板线程（如果模板线程还未运行）
// 调用线程（调用方）它自己必须处于正常状态
func startTemplateThread() {
	if GOARCH == "wasm" { // wasm架构还未有线程
		return
	}
	...
	if !atomic.Cas(&newmHandoff.haveTemplateThread, 0, 1) {
		releasem(mp) //模板线程templateThread已启动，因此把获取到的m释放
		return
	}
...
// 如果传入的p是nil，则尝试从P空闲池里取P，若无空闲P则啥都不干就return
// 如果传入spinning=true，则调用者自增了nmspinning，
// _p_:m要绑定的p
// spinning：当没有_p_可给m绑定时，是否可以接受m自旋
//go:nowritebarrierrec
func startm(_p_ *p, spinning bool) {
	...
	mp := acquirem() //标记当前m上所执行的g不可被抢占。也即下面的代码逻辑会一直占有cpu执行
	lock(&sched.lock)
	if _p_ == nil { //如果传入的p为nil，则从空闲P池里取P
		_p_ = pidleget() //尝试获取空闲的p
		if _p_ == nil {  //无空闲的p，那么mp只能自旋 或 return了
			unlock(&sched.lock)
			if spinning { //M可以自旋等待
				// The caller incremented nmspinning, but there are no idle Ps,
				// so it's okay to just undo the increment and give up.
				// 调用方自增了nmspinning，但没有空闲P，所以撤掉自增的nmspinning也没关系，然后放弃
				if int32(atomic.Xadd(&sched.nmspinning, -1)) < 0 {
					throw("startm: negative nmspinning")
				}
			}
			releasem(mp) //使得当前g恢复可被抢占态
			return
		}
	}
	...
	unlock(&sched.lock)
	if nmp.spinning { //如果m在自旋（说明该m并不空闲）
		throw("startm: m is spinning")
	}
	if nmp.nextp != 0 { //如果m已有p（说明该m并不空闲）
...
// 从系统调用 或 被锁的M 里释放P（即M因为系统调用或被阻塞，此时把M绑定的P解绑），该函数的运行都不需要持有P，（也即是系统线程来执行的）所以不允许开启写屏障
//go:nowritebarrierrec
func handoffp(_p_ *p) {
	...
	// 如果_p_的本地队列不为空 或 全局队列里有G，那么通过调用startm，分配另一个m和_p_绑定，然后执行g
	if !runqempty(_p_) || sched.runqsize != 0 {
		startm(_p_, false)
		return
	}
	// if it has GC work, start it straight away
	// 如果有GC处于标记阶段且有对象需要被标黑，那么_p_就被派去执行该工作
	...
	if atomic.Load(&sched.nmspinning)+atomic.Load(&sched.npidle) == 0 && atomic.Cas(&sched.nmspinning, 0, 1) { // TODO: fast atomic
		//既没空闲的p，也没自旋的线程m，那就创建一个新线程m来绑定p
		startm(_p_, true)
		return
	}
	lock(&sched.lock)
	...
	if _p_.runSafePointFn != 0 && atomic.Cas(&_p_.runSafePointFn, 1, 0) {
		sched.safePointFn(_p_) //在p上运行saftpoint函数，用以gc
		sched.safePointWait--
		if sched.safePointWait == 0 {
			notewakeup(&sched.safePointNote)
		}
...
// Returns with acquired P.
// 当g和m互相绑定了，这意味着当前g正在执行会阻塞系统线程m的调用，所以此时线程m也会被挂起
// 停止某个被g绑定的m的运行，直到该g再次可执行时，m恢复运行，这种情况常见于g触发了一个系统调用等（此时gm互相绑定了）
func stoplockedm() {
	_g_ := getg()
...
func startlockedm(gp *g) {
	...
	mp := gp.lockedm.ptr() //用户协程gp上的m
	if mp == _g_.m {
		throw("startlockedm: locked to me")
	}
	if mp.nextp != 0 { //mp还有任务的
		throw("startlockedm: m has p")
	}
	// directly handoff current P to the locked m
	incidlelocked(-1)
	_p_ := releasep()    //把g0上的p和g0上的m解绑，解绑后，p处于空闲态
	mp.nextp.set(_p_)    //把g0上的p绑定到用户协程gp上的m
	notewakeup(&mp.park) //唤醒用户协程gp上的m
	stopm()              //挂起/暂停g0上的m
}

// Stops the current m for stopTheWorld.
// Returns when the world is restarted.
// 暂停当前m以实现 stw ，当stw恢复时才返回
func gcstopm() {
	_g_ := getg()
...
func execute(gp *g, inheritTime bool) {
	...
	//当gp被阻塞时，waitsince为阻塞开始的时间点
	gp.waitsince = 0
	gp.preempt = false                         //gp当前不可被抢占
	gp.stackguard0 = gp.stack.lo + _StackGuard //栈顶是处于低地址，且距离 stack.lo 还差几个字节
	if !inheritTime {                          //是否把上一个g未用完的时间片用在本g上
		// inheritTime=false，说明用新时间片，那么调度器的调度次数增加 1
		_g0_.m.p.ptr().schedtick++
	}
	//inheritTime=true，说明用回旧时间片

	// Check whether the profiler needs to be turned on or off.
	hz := sched.profilehz
	if _g0_.m.profilehz != hz {
	...
	//gogo完成从g0到用户协程gp的切换，调度gp以继续执行gp的逻辑
...
// 把gList里的每个runnable的G加入到run队列里并清除glist
// 如果当前执行本函数时，没有绑定的P（意味着本函数可能在g0里执行），
// 则把它们都加到全局run队列，并且尝试用启动M绑定P来处理它们（M仍要等待有P才可执行），
// 如果有空闲P，那么调度器只会将 glist 中的一个 G 添加到全局队列，
// 并启动空闲M绑定P来执行该G，glist中剩余的G，则会被添加到当前P的本地队列中，等待这个P后续处理。
// 该函数会临时获取shec.lock锁，且该函数可和GC并发执行
func injectglist(glist *gList) {
	...
	// on the run queues.
	// 把每个g放入gQueue之前，都标记为runnable
	head := glist.head.ptr() //gList 队头
	var tail *g
	qsize := 0                                           //glist的大小
	for gp := head; gp != nil; gp = gp.schedlink.ptr() { //gp指向glist的下一个g
		tail = gp
		qsize++
		casgstatus(gp, _Gwaiting, _Grunnable) //把glist里的每个g都set为runnable
	}

	// 把gList转为gQueue
	var q gQueue
	q.head.set(head) //gQueue的头 指向 glist的头
	q.tail.set(tail) //gQueue的尾 指向 glist的尾
	*glist = gList{} //清空glist

	startIdle := func(n int) { //调度m去绑定空闲的p来执行g
		for ; n != 0 && sched.npidle != 0; n-- { //尝试n次
			startm(nil, false) //调度m去绑定空闲的p（因为传入的p=nil）来执行g
		}
	}

	pp := getg().m.p.ptr() //获取当前g（当前运行着本代码的g）的m所绑定的p
	if pp == nil {         //因为m0不需要绑定p，所以当pp=nil则说明当前g是g0
		lock(&sched.lock)
		//把q里的qsize个g放到全局队列
		...
		startIdle(qsize) //尝试qsize次去启动m绑定P处理全局队列里的g，当没有空闲p时，依旧无法处理则先退出
		return
	}
...
// Never returns.
// 调度器的一轮调度：寻找一个runnable的g并执行它，本函数不会返回
func schedule() {
	_g_ := getg() //每个线程m对应的g0，初始化时是m0的g0

	if _g_.m.locks != 0 { //当前m0被锁住（休眠ing）
		throw("schedule: holding locks")
	}

	if _g_.m.lockedg != 0 { //如果当前m上绑定了某个g
		stoplockedm() //则把m挂起，以下逻辑则暂停执行

		//到此 m被重新唤醒运行

		//然后调起lockedg指向的g运行
		execute(_g_.m.lockedg.ptr(), false) // execute函数不会return
	}

	// We should not schedule away from a g that is executing a cgo call,
	// since the cgo call is using the m's g0 stack.
	// 当m正在执行cgo调用时，不应该把该m上的g调度走，因为cgo调用是使用m的g0栈的，也即此时是在g0栈里执行cgo调用
	if _g_.m.incgo {
		//当前m正在执行cgo调用（也即此时正在运行的g是g0），g0不可被调度，报异常
		throw("schedule: in cgo")
	}

top:
	pp := _g_.m.p.ptr() //当前m绑定的p
	pp.preempt = false  //

	if sched.gcwaiting != 0 { //需要执行gc了
		gcstopm() //暂停m，也即stw，然后阻塞在这，直到stw结束时，才返回
		goto top
	}
	...
	if _g_.m.spinning && (pp.runnext != 0 || pp.runqhead != pp.runqtail) { //当前m在自旋，并且p上还有g要执行
		//则异常，因为当前m所绑定的p当前有g需要执行的话，该m不可能处于自旋态
		throw("schedule: spinning with local work")
	}

	checkTimers(pp, 0) //检查当前p是否有定时器要运行
...
// 本函数是解除当前 M（OS 线程）和当前运行的 G（用户协程）之间的绑定
...
func dropg() {
	_g_ := getg() //返回的是g0，注意_g_.m是当前g0所在的M，这个M原本是绑定了用户协程的，现在要解绑它们两
	//以下两步是把当前M的用户协程G（非g0）和当前M解绑/分离
	setMNoWB(&_g_.m.curg.m, nil) // &_g_.m.curg 指向的是用户协程G，把当前M上的用户协程(curg)的m置空，表示它不再被当前M（线程）持有。
	setGNoWB(&_g_.m.curg, nil)   //把 M 里的 curg 也清空，表示 M 目前没有关联的用户协程。
}
...
func park_m(gp *g) {
	...
	if trace.enabled {
		traceGoPark(_g0_.m.waittraceev, _g0_.m.waittraceskip)
	}

	//把当前的用户协程gp从running态改为waiting态
	casgstatus(gp, _Grunning, _Gwaiting)
	//把当前M和M所绑定的用户协程gp解绑，因为gp不再是running态了，此时gp真正被挂起了
	dropg()

	//为当前M找一个新的runnable的用户协程G

	//先看看当前M有无阻塞的等待
	if fn := _g0_.m.waitunlockf; fn != nil { //表示当前线程M上有阻塞等待事件
		ok := fn(gp, _g0_.m.waitlock)
		_g0_.m.waitunlockf = nil
		_g0_.m.waitlock = nil
		if !ok {
			if trace.enabled {
				traceGoUnpark(gp, 2)
			}
			casgstatus(gp, _Gwaiting, _Grunnable) //完成 fn 函数的执行后，用户协程gp重新设为runnable态，等待下一次调用执行
			execute(gp, true)                     // 重新调用用户协程gp的执行，该函数不会返回 Schedule it back, never returns.
		}
	}
	schedule() //调度其他用户协程执行
}
...
//
// 系统调用syscall的入口，被用于go的syscall包和普通的cgo调用
// 通过 linkname 的方式暴露给syscall包，以进行编译
//go:nosplit
...
//
// This is exported via linkname to assembly in the syscall package.
// 当g退出系统调用的阻塞（系统线程m也退出阻塞）时，该函数会被调用
// 该函数的调用只会由 syscall 包里的函数调用
// 不可以写屏障
//go:nosplit
//go:nowritebarrierrec
...
// 新建一个带有stacksize字节栈空间的g实例
func malg(stacksize int32) *g {
	newg := new(g)      //新建实例
	if stacksize >= 0 { //分配g的栈空间
		stacksize = round2(_StackSystem + stacksize)
		systemstack(func() {
			newg.stack = stackalloc(uint32(stacksize)) //分配一个stacksize字节的栈
		})
		newg.stackguard0 = newg.stack.lo + _StackGuard //初始化stackguard0的零值
		newg.stackguard1 = ^uintptr(0)                 //1111 1111
...
func newproc(fn *funcval) {
	gp := getg()
	pc := getcallerpc()  //返回 调用go关键字的函数 的pc（该函数当前执行到的指令的地址）
	systemstack(func() { //切换到g0执行下列逻辑
		newg := newproc1(fn, gp, pc) //创建一个新g，fn是要运行的函数，gp是当前协程，pc

		//如果要判断当前正在系统栈上执行还是用户栈上执行，可以使用getg() == getg().m.curg

		_p_ := getg().m.p.ptr()
		runqput(_p_, newg, true) //当创建新的协程newg出来时，放入g0所在的p的runq队列里，
...
// 创建一个状态为可运行态的新g实例，从fn函数（入口函数）开始执行，
// callerpc是创建新协程的go语句的地址（在父协程内），调用者负责把新g添加到调度器
func newproc1(fn *funcval, callergp *g, callerpc uintptr) *g {
	...
	acquirem() // 关闭可抢占标识，disable preemption because it can be holding p in a local var

	_p_ := _g_.m.p.ptr() //当前的P
	newg := gfget(_p_)   //从p里申请一个g（可能从空闲g池复用或新建实例g）
	if newg == nil {
		newg = malg(_StackMin) //新建一个g实例，并分配2k栈空间
	...
	sp := newg.stack.hi - totalSize //sp是栈顶，栈是从高地址开始，往低地址生长的
	spArg := sp
	if usesLR {
		// caller's LR
		*(*uintptr)(unsafe.Pointer(sp)) = 0
		prepGoExitFrame(sp)
		spArg += sys.MinFrameSize //最小帧的大小
	}

	memclrNoHeapPointers(unsafe.Pointer(&newg.sched), unsafe.Sizeof(newg.sched)) //清零newg.sched指向的空间（为什么要清零？因为newg可能是复用来的g）
	newg.sched.sp = sp
	newg.stktopsp = sp
	newg.sched.pc = abi.FuncPCABI0(goexit) + sys.PCQuantum // +PCQuantum so that previous instruction is in same function
	newg.sched.g = guintptr(unsafe.Pointer(newg))
	gostartcallfn(&newg.sched, fn) //把fn指向的函数地址写入到新g的sched字段里（如pc和sp等）
	newg.gopc = callerpc           //记录go关键字的位置地址
	newg.ancestors = saveAncestors(callergp)
	newg.startpc = fn.fn //新协程newg的入口函数
	...
	gcController.addScannableStack(_p_, int64(newg.stack.hi-newg.stack.lo)) //统计本p已分配的栈空间大小，newg.stack.hi-newg.stack.lo 栈大小
...
// 从_p_的本地gfree链表里获取一个g，如果本地gfree为空，则从全局链表里获取一批
// 步骤：
// 1.如果p本地gfree为空，则从全局sched的gfree里取出32个带有栈空间或不带栈空间的g放到p的本地gfree里
// 2、从p本地gfree里pop出一个p
// 3、如果g还没申请栈空间，则申请
// 4、返回g
func gfget(_p_ *p) *g {
retry:
	...
	_p_.gFree.n--
	if gp.stack.lo == 0 { //该g还没有申请栈空间
		// Stack was deallocated in gfput. Allocate a new one.
		systemstack(func() {
			gp.stack = stackalloc(_FixedStack) //申请一个大小为_FixedStack的栈空间
		})
		gp.stackguard0 = gp.stack.lo + _StackGuard
	} else {
		if raceenabled {
...
// 本函数的作用是把调用者（当前g）绑定到当前m上
// 本函数由 LockOSThread 和 lockOSThread 调起，本函数执行期间不允许被抢占，否则本函数内的m和调用者的m可能会不同
//go:nosplit
func dolockOSThread() {
	//wasm架构不支持线程绑定
	if GOARCH == "wasm" {
	...
	_g_ := getg()
	_g_.m.lockedg.set(_g_) //该_g_绑定了当前m
	_g_.lockedm.set(_g_.m) //当前m绑定了该_g_
}
...
//该函数和lockOSThread的区别是，本函数供外部使用，lockOSThread供内部使用
func LockOSThread() {
...
//剩余M的个数
func mcount() int32 {
	return int32(sched.mnext - sched.nmfreed) //已创建M的个数 - 已释放M的个数 = 剩余M的个数
}
...
// 检查当前死锁 deadlock 的情况 Check for deadlock situation.
// 基于正在运行的M的数量来检查是否死锁，当0个M在运行时，则死锁 The check is based on number of running M's, if 0 -> deadlock.
// 需先持有 sche.lock锁 才能调用本函数
func checkdead() {
	assertLockHeld(&sched.lock) //是否持有sched.lock锁，是才能跑下去，否则断言中断
	...
	run := mcount() - sched.nmidle - sched.nmidlelocked - sched.nmsys //已创建M的个数 - 已释放M的个数 - 空闲M的个数 - 被锁住的M的个数 - 不计入死锁的M的个数
	if run > run0 {
		return
	}
	if run < 0 {
	...
	grunning := 0
	forEachG(func(gp *g) { //每个G都执行该匿名函数
		if isSystemGoroutine(gp, false) {
			return
		}
		s := readgstatus(gp)
...
func sysmon() {
	...
	checkdead()   //检查当前死锁情况，若出现死锁，则直接报错退出进程
	unlock(&sched.lock)
	...
	for { //系统监控sysmon循环执行，每次睡眠delay微秒
		...
		if netpollinited() && lastpoll != 0 && lastpoll+10*1000*1000 < now { //已经有10ms没调用netpoll了
			...
			if !list.empty() {
				...
				// 需要自减空闲且被锁的M的数量
				incidlelocked(-1)
				injectglist(&list) //把list里的g放入全局队列或本p的本地队列
				incidlelocked(1)
			}
		}
...
// 广播全部goroutine他们已被抢占并且要停止，该函数是尽量尝试，如果一个goroutine刚被p调起执行，则可能无法让该goroutine停止
// 调用该函数时无需先持有锁
// 如果给哪怕一个goroutine发出抢占请求，都返回true
func preemptall() bool {
	res := false
	for _, _p_ := range allp {
		if _p_.status != _Prunning { //p不是running，则说明该p上无g在执行
			continue
		}
		if preemptone(_p_) {
			res = true
...
// sched.lock must be held.
// May run during STW, so write barriers are not allowed.
// 把m放回 空闲m池，调用本函数前需持有锁，可在STW时运行，因此写屏障是不允许的
//go:nowritebarrierrec
func mput(mp *m) {
	assertLockHeld(&sched.lock) //断言有无持有锁，无则报错
...
// May run during STW, so write barriers are not allowed.
// 把一批runnable的g批量放到全局队列，并清除batch。
// 调用本函数前必须持有全局lock锁
// 次函数可能在STW时执行（此时是在g0）因此写屏蔽是不允许开启的
//go:nowritebarrierrec
func globrunqputbatch(batch *gQueue, n int32) {
	assertLockHeld(&sched.lock) //先判断是否持有锁，调用本函数前必须先持有sched.lock

	sched.runq.pushBackAll(*batch) //batch新队列加入到全局队列尾部
	sched.runqsize += n
	*batch = gQueue{} //清理batch
}
...
// 尝试从 _Pidle 空闲P池里取P
// 需持有shec.lock锁
// 可能在STW期间运行，所以写屏障是不能开启的
// 可能返回nil
func pidleget() *p {
	assertLockHeld(&sched.lock) //如果调用方没持有锁，则不能调用本函数

	_p_ := sched.pidle.ptr()
	if _p_ != nil { //有空闲的p（没有m绑定的p）
		// Timer may get added at any time now.
		timerpMask.set(_p_.id)
		idlepMask.clear(_p_.id)
		sched.pidle = _p_.link         //sched.pidle指向空闲P池里的下一个P
		atomic.Xadd(&sched.npidle, -1) // TODO: fast atomic
	}
	return _p_
}
...
// 尝试把g放入_p_的本地队列
// 如果next=false，则把g放入队尾
// 如果next=true，则把g放到p.runnext字段（即下一个就要执行）
// 如果队列满了，则放入到全局队列
...
retry:
	...
	t := _p_.runqtail
	if t-h < uint32(len(_p_.runq)) { //队列未满
		_p_.runq[t%uint32(len(_p_.runq))].set(gp) //把gp放入队尾
		atomic.StoreRel(&_p_.runqtail, t+1)       // store-release, makes the item available for consumption
		return
	}

	//队列满了
	if runqputslow(_p_, gp, h, t) { //把gp和P本地队列的前一半的g放入到全局队列（因为p的本地队列满了）
		return
	}

	// the queue is not full, now the put above must succeed
	// 在执行runqputslow期间，可能本地队列又有空间了，就再尝试把gp放入本地队列
	goto retry
}

// Put g and a batch of work from local runnable queue on global queue.
// Executed only by the owner P.
// 把gp和P本地队列的前一半的g放入到全局队列（因为p的本地队列满了）
func runqputslow(_p_ *p, gp *g, h, t uint32) bool {
	var batch [len(_p_.runq)/2 + 1]*g //临时队列
...
// not be used.
// 把q2添加到q的尾部，本方法调用后，q2必须清掉不能继续使用
func (q *gQueue) pushBackAll(q2 gQueue) {
	...
	if q.tail != 0 { //q不为空
		q.tail.ptr().schedlink = q2.head //q的尾部指向q2的头部
	} else {
		q.head = q2.head
	}
	q.tail = q2.tail
