...
type service struct {
	name   string                 // 服务名 name of service
...
func (server *Server) ServeCodec(codec ServerCodec) {
	...
	for {
		...
		wg.Add(1)
		go service.call(server, sending, wg, mtype, req, argv, replyv, codec) //同一个连接上的多个rpc请求，分别建一个goroutine去处理
	}
...
func (server *Server) Accept(lis net.Listener) {
	for {
		...
		go server.ServeConn(conn) //每个新的rpc连接
	}
}
